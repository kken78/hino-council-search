...
const clean = (s) => {
  ...
  // 1) 先頭の「－4－」ページヘッダ
  r = r.replace(/^[－―‐\-]+[0-9０-９]+[－―‐\-]+/, "");
  // 2) 先頭の「2-56」「3-36」=ページ-行番号。後ろに議席番号が続けばそれは残す
  r = r.replace(/^[0-9０-９]+[-－―‐][0-9０-９]+(?=[0-9０-９]*番)/, ""); // 「2-5610番」→「10番」
  r = r.replace(/^[0-9０-９]+[-－―‐][0-9０-９]+/, "");                  // 「2-5農林課長」→「農林課長」
  // 3) 先頭・中間のト書き「－退席－」等（役職名にこれらの記号は無いので最後の区切りまで捨てる）
  r = r.replace(/^(?:[－―‐][^－―‐]*[－―‐])+/, "");
  if (/[－―‐]/.test(r)) r = r.replace(/^.*[－―‐]/, "");
  if (/[〔〕〈〉《》【】]/.test(r)) r = r.replace(/^.*[〔〕〈〉《》【】]/, "");
  // 4) 議席番号を半角に統一（\s は空白0個以上）
  r = r.replace(/([0-9０-９]+)\s*番/g, (_, n) => n.replace(/[０-９]/g, d => "０１２３４５６７８９".indexOf(d)) + "番");
  // 5) 先頭に浮いた単独数字（直後が役職名）。議席番号「10番」は (?=…番) で保護済み
  r = r.replace(/^[0-9０-９]+(?=[^0-9０-９番])/, "");
  return r.trim();
};
