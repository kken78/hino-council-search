...
```
 ...
 (会議録)   │ 2.extract  PDF → テキスト（pdfjs）                                            │
        ...
        ┌──────────────── 公開（GitHub Pages・静的）────────────────┐
        │  React/Vite フロント： index.json を読み、クライアント側で  │
...
│  ├─ extract.mjs
│  ├─ split.mjs
│  ├─ enrich.mjs        # 任意（Claude）
│  ├─ embed.mjs         # 任意（埋め込み）
│  ├─ index.mjs
│  └─ lib/
│     ├─ normalize.mjs  # NFKC＋異体字＋空白除去
│     ├─ eras.mjs       # 元号⇔西暦
│     └─ speech.mjs     # 発言分割・議案抽出
├─ dict/                                   # 辞書（手で育てる）
│  ├─ yomi.json
│  ├─ synonyms.json
│  └─ variants.json
├─ data/                                   # 中間生成物（raw text 等）
│  ├─ pdf/                                 # ダウンロードPDF（キャッシュ）
│  └─ text/                                # 抽出テキスト
...
Vite + React。`public/data/*.json` を起動時に取得し、クライアントで正規化検索（[1.1](#11-検索)〜[1.2](#12-表記ゆれ吸収正規化)）。セマンティック検索ONのときはキーワード結果と意味結果を統合（ハイブリッド：キーワード一致を優先しつつ意味的近傍を補完）。プロトタイプ（このリポジトリの `src/App.jsx`）が参照実装。
...
```
> Settings → Pages を「GitHub Actions」に設定。Vite の `base` をリポジトリ名に合わせる（例：`base: "/gikai-search/"`）。
...
- [ ] 全会議録の自動取り込み（巡回〜索引の本番運用）
- [ ] 漢数字⇔算用数字の正規化、辞書の継続拡充
